...
async fn registry_extensions_appear_in_openapi_paths() {
    ...
    let spec = oxipage_core::openapi::openapi_spec("http://localhost:8787", &registry);
    ...
    assert!(paths.contains_key("/api/console/setup/extension-step/{id}"));
...
async fn empty_registry_omits_extension_paths() {
    ...
    let spec = oxipage_core::openapi::openapi_spec("", &registry);
    let paths = spec["paths"].as_object().expect("paths is object");
