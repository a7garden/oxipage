...
use oxibuilder_core::builder::{BuildInputs, BuildOutput, StaticPage};
...
fn relative_assets_drop_leading_slash() {
    ...
    assert!(!html.contains("\"/assets/"), "raw /assets/ leaked: {html}");
    assert!(html.contains("\"assets/"), "relative asset missing: {html}");
    assert!(
