...
async fn admin_html_has_revision_meta_and_header() {
    ...
    let header_rev = resp
        .headers()
        .get("X-Oxibuilder-SPA-Revision")
        .expect("X-Oxibuilder-SPA-Revision header must be set on admin.html")
        .to_str()
        .expect("X-Oxibuilder-SPA-Revision must be ASCII")
        .to_owned();
    assert!(
        !header_rev.is_empty(),
        "X-Oxibuilder-SPA-Revision must be non-empty"
    );
    ...
    let meta_rev = body
        .split("name=\"oxibuilder-spa-revision\" content=\"")
        .nth(1)
        .and_then(|s| s.split('\"').next())
        .expect("admin.html must carry the oxibuilder-spa-revision meta tag");
    assert_eq!(
        meta_rev, header_rev,
        "meta tag content and X-Oxibuilder-SPA-Revision header must agree"
    );
}
