...
pub struct CliCommand {
    ...
    pub name: &'static str,
    /// `oxibuilder novels --help` 상단에 표시될 설명.
    pub about: &'static str,
    /// 이 명령의 하위 서브커맨드들.
    pub subcommands: Vec<CliSubcommand>,
}

/// 단일 서브커맨드 (예: "oxibuilder novels new").
#[derive(Clone)]
...
pub trait Extension: Send + Sync {
    /// 고유 식별자. oxibuilder.toml의 enabled 목록, API 경로 프리픽스, 로비 매니페스트 키로 재사용.
    fn id(&self) -> &str;
...
/// 코어(`oxibuilder-core`)는 wasmtime 에 의존하지 않으므로, 실제 로딩은 이 trait의
/// 구현체(`oxibuilder-wasm`)가 담당한다. 서버가 `--features wasm` 으로 빌드되었을 때
