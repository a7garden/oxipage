...
/// 예: `oxibuilder-1735689600.db`.
pub fn snapshot_filename(epoch_secs: u64) -> String {
    format!("oxibuilder-{epoch_secs}.db")
}
...
mod tests {
    ...
    fn filename_is_deterministic() {
        assert_eq!(snapshot_filename(1735689600), "oxibuilder-1735689600.db");
    }

    #[tokio::test]
    async fn vacuum_into_creates_file() {
        let dir =
            std::env::temp_dir().join(format!("oxibuilder-backup-test-{}", std::process::id()));
