...
//!
//! File I/O (`load`/`save` with permissions) lives in `oxibuilder` (CLI crate)
//! under `crate::sites`. This module provides only the type definitions plus
//! in-memory data access.
//!
//! v2 SSG: each site = local directory with its own `oxibuilder.toml` + `oxibuilder.db`.
...
/// Top-level `~/.config/oxibuilder/sites.toml` file.
...
/// One named site entry — a local oxibuilder project directory.
...
impl SitesFile {
    /// Resolve the effective site name: `--site` flag → OXIBUILDER_SITE env →
    /// `default_site` → `None`.
    ///
