...
// 환영 글 (샘플 post) 데이터는 `oxibuilder-ext-blog`가 자기 `seed_sample_data()`에서
// 정의/관리한다. 코어는 블로그 도메인 데이터를 더 이상 모른다.
// Setup's theme list is now sourced from the shared `oxibuilder_core::theme`
...
fn update_toml_site(name: &str, base_url: &str) -> anyhow::Result<()> {
    let config_path = std::env::var("OXIBUILDER_CONFIG")
        ...
        .unwrap_or_else(|_| std::path::PathBuf::from("oxibuilder.toml"));
...
///
/// **Deprecated in favor of `oxibuilder_console::console_state::ConsoleState::is_setup_needed`.**
