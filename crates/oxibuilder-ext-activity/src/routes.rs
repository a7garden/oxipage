...
/// `OXIBUILDER_GITHUB_WEBHOOK_SECRET`으로 검증한다.
///
...
pub async fn webhook(
    ...
    let secret = std::env::var("OXIBUILDER_GITHUB_WEBHOOK_SECRET")
        .ok()
        .filter(|s| !s.is_empty())
        .ok_or_else(|| {
            ApiError::new(
                ...
                "GitHub webhook requires OXIBUILDER_GITHUB_WEBHOOK_SECRET",
            )
        })?;
...
pub async fn sync(
    ...
    let client = GithubClient::with_username(std::env::var("OXIBUILDER_GITHUB_USERNAME").ok())
        .map_err(ApiError::internal)?;
    if !client.enabled() {
        return Err(ApiError::new(
            StatusCode::SERVICE_UNAVAILABLE,
            "integration_disabled",
            "GitHub activity sync requires OXIBUILDER_GITHUB_USERNAME",
        ));
    }
    let events = client
        .fetch_public_events()
