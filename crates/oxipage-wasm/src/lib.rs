...
fn host_db_query_impl(caller: &mut Caller<'_, WasmHostState>, sql_ptr: i32, sql_len: i32) -> i64 {
    ...
    if !trimmed.to_ascii_uppercase().starts_with("SELECT") {
        tracing::warn!(target: "oxipage::wasm::capability", "host_db_query rejected: not SELECT");
        return 0;
    }

    // 3. DB pool 확인.
    let Some(db) = &caller.data().db.clone() else {
        tracing::warn!(target: "oxipage::wasm::capability", "host_db_query: no db in store");
        return 0;
    };
    ...
    let json = match json_result {
        Ok(j) => j,
        Err(e) => {
            tracing::warn!(target: "oxipage::wasm::capability", error = %e, "host_db_query failed");
            return 0;
        }
    };
...
fn host_http_get_impl(caller: &mut Caller<'_, WasmHostState>, url_ptr: i32, url_len: i32) -> i64 {
    ...
    let body = match body_result {
        Ok(b) => b,
        Err(e) => {
            tracing::warn!(target: "oxipage::wasm::capability", error = %e, "host_http_get failed");
            return 0;
        }
    };
    ...
    let body = if body.len() > MAX_CAPABILITY_RESPONSE {
        tracing::warn!(target: "oxipage::wasm::capability", "host_http_get response truncated");
...
fn write_capability_result(caller: &mut Caller<'_, WasmHostState>, data: &[u8]) -> i64 {
    ...
    let Some(alloc_func) = caller.get_export("alloc").and_then(|e| e.into_func()) else {
        tracing::warn!(target: "oxipage::wasm::capability", "module missing alloc export");
        return 0;
    };
