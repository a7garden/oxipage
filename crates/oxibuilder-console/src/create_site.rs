//! Create-site handler — seeds a new oxibuilder project directory and
...
/// Resolve ~/.config/oxibuilder/sites.toml path.
...
"#;
        std::fs::write(path.join("oxibuilder.toml"), toml_content)
            .map_err(|e| format!("cannot write oxibuilder.toml: {e}"))?;
    }
