...
use oxibuilder_core::config::ServerConfig;
use oxibuilder_core::site_paths::{GitHubPagesTarget, MutableSiteSettings};
...
pub(crate) async fn atomic_write_and_reload(
    ...
    let new_cfg = oxibuilder_core::config::Config::load(&toml_path)
...
async fn evaluate_preflight(ctx: &SiteContext) -> DeployPreflight {
    ...
    let origin = match (&target, git) {
        (Some(t), true) => std::process::Command::new("git")
            ...
            .is_some_and(|o| {
                oxibuilder_deploy::origin_matches(&String::from_utf8_lossy(&o.stdout), t)
            }),
        _ => false,
    };
    ...
    let manifest = oxibuilder_core::build_manifest::BuildManifest::read_from(&ctx.out_dir)
        .ok()
        .flatten();
    if manifest.is_none() {
...
pub async fn extensions_list(
    ...
    for ext in ctx.registry.iter() {
        ...
        out.push(ExtensionInfo {
            id: ext.id().to_string(),
            display_name: ext.display_name(oxibuilder_core::extension::Lang::En),
