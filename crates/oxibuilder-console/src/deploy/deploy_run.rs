...
use oxibuilder_core::site_paths::GitHubPagesTarget;
use oxibuilder_deploy::{DeployError, DeployEvent};
use sqlx::SqlitePool;
use std::sync::Arc;
...
pub async fn ensure_deploy_started(
    ...
    let manifest = match oxibuilder_core::build_manifest::BuildManifest::read_from(&ctx.out_dir) {
        Ok(Some(m)) => m,
        Ok(None) => {
    ...
    tokio::spawn(async move {
        ...
        let outcome: Result<oxibuilder_deploy::DeployOutcome, oxibuilder_deploy::DeployError> =
            ...
            .await
            .map_err(|e| {
                oxibuilder_deploy::DeployError::Io(std::io::Error::other(format!(
        ...
        let (status, url, commit, code, error) = match &outcome {
            Ok(oxibuilder_deploy::DeployOutcome::Deployed { url, commit }) => (
                "deployed",
                Some(url.clone()),
                Some(commit.clone()),
                None,
            ...
            Ok(oxibuilder_deploy::DeployOutcome::Unchanged { url, commit }) => (
                "unchanged",
                Some(url.clone()),
                Some(commit.clone()),
                None,
