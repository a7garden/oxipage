...
async fn create_site_handler_seeds_toml_and_returns_slug() {
    let tmp = TempDir::with_prefix("oxibuilder-create-").unwrap();
    let target = tmp.path().join("blog");

    let registry = Arc::new(
        SiteRegistry::new(SitesFile::default(), Default::default())
            .await
            .unwrap(),
    );
    ...
    assert!(
        target.join("oxibuilder.toml").exists(),
        "oxibuilder.toml should exist"
    );
}
