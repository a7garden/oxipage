...
fn create_site_dir(name: &str, toml_text: &str) -> (TempDir, PathBuf) {
    let dir = TempDir::with_prefix(format!("oxibuilder-cfg-{name}-")).unwrap();
...
async fn deploy_patch_preserves_server_and_unknown_keys() {
    ...
    let saved: toml::Value =
        toml::from_str(&std::fs::read_to_string(project_dir.join("oxibuilder.toml")).unwrap())
            .unwrap();
...
async fn invalid_target_is_not_persisted() {
    ...
    let path = project_dir.join("oxibuilder.toml");
