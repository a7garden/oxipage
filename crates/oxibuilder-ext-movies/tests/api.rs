...
async fn tmdb_search_disabled_when_no_key() {
    ...
    if std::env::var("OXIBUILDER_TMDB_KEY").is_ok() {
        eprintln!("OXIBUILDER_TMDB_KEY is set; skipping tmdb_disabled assertion");
        return;
    }
    let app = test_app(Some("tok")).await;
    let res = app
...
async fn fts_index_on_publish() {
    ...
    use oxibuilder_ext_movies::model::{MovieEntryInput, MovieEntryPatch};
    use oxibuilder_ext_movies::repo;
    ...
    let hits = oxibuilder_core::search::search(&pool, "dreams", None, 10)
        .await
        .unwrap();
