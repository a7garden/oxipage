...
pub enum SiteCommand {
    /// 새 사이트 등록 (로컬 oxibuilder 프로젝트 디렉토리)
    Add {
        name: String,
        #[arg(long)]
        path: String,
...
fn site_list(
    ...
    if out.json {
        ...
        if names.is_empty() {
            println!("no sites configured — use `oxibuilder site add`");
            return Ok(());
        }
        for name in &names {
            let entry = sites_file.get(name);
...
async fn site_add(
    ...
    if sites_file.exists(name) {
        anyhow::bail!("site '{name}' already exists — use `oxibuilder site edit` to update");
    }
    let site_path = PathBuf::from(path);
    if !site_path.exists() {
        anyhow::bail!("path '{path}' does not exist");
    }
    if !site_path.join("oxibuilder.toml").exists() {
        anyhow::bail!("path '{path}' is not an oxibuilder project (no oxibuilder.toml found)");
    }
    let mut new_sites = sites_file.clone();
    new_sites
        .sites
...
async fn site_edit(
    ...
    let _entry = sites_file.sites.get(name).ok_or_else(|| {
        anyhow::anyhow!("site '{name}' not found — use `oxibuilder site add` first")
    })?;
    let mut new_sites = sites_file.clone();
