...
pub(crate) async fn deploy(c: DeployArgs, out: &Output) -> anyhow::Result<()> {
    ...
    // default), falling back to the legacy oxibuilder.toml path.
    let sites = crate::sites::load_sites();
    let legacy = std::env::var_os("OXIBUILDER_CONFIG")
        .map(PathBuf::from)
        .or_else(|| {
            Path::new("oxibuilder.toml")
                .exists()
                .then(|| PathBuf::from("oxibuilder.toml"))
        });
    let project = resolve_deploy_project(c.site.as_deref(), &sites, legacy.as_deref())?;
    let cfg = oxibuilder_core::config::Config::load(&project.join("oxibuilder.toml"))?;
    ...
    match c.target.as_str() {
        "github-pages" => {
            let target = cfg
                ...
                .ok_or_else(|| anyhow::anyhow!("[deploy.github_pages] is not configured"))?;
            target.validate()?;
            let manifest = oxibuilder_core::build_manifest::BuildManifest::read_from(&out_dir)?
                .ok_or_else(|| {
                    anyhow::anyhow!("build required: no manifest in {}", out_dir.display())
                })?;

            let (tx, mut rx) = tokio::sync::mpsc::channel::<oxibuilder_deploy::DeployEvent>(32);
            ...
            match handle.await {
                Ok(Ok(oxibuilder_deploy::DeployOutcome::Deployed { url, commit })) => {
                    out.ok(format!("deployed {commit} to {url}"))
                }
                Ok(Ok(oxibuilder_deploy::DeployOutcome::Unchanged { url, commit })) => {
...
/// wins; otherwise fall back to the legacy oxibuilder.toml path's parent.
pub fn resolve_deploy_project(
    requested: Option<&str>,
    sites: &oxibuilder_core::sites::SitesFile,
    legacy: Option<&Path>,
    ...
    legacy
        ...
        .ok_or_else(|| anyhow::anyhow!("no registered site and no oxibuilder.toml"))
}

fn deploy_event_label(ev: &oxibuilder_deploy::DeployEvent) -> String {
    use oxibuilder_deploy::DeployEvent;
    match ev {
    ...
    fn legacy_only_without_registry() {
        assert_eq!(
            resolve_deploy_project(
                None,
                &SitesFile::default(),
                Some(Path::new("/legacy/oxibuilder.toml"))
            )
            .unwrap(),
            PathBuf::from("/legacy")
        );
