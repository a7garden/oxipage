# Oxibuilder v2 — Static Site Generator

> 설계 문서. 2026-07-28.

## 1. 동기

Oxibuilder v1은 상시 가동 서버(`oxibuilder-core`)에 의존한다. 블로그·포트폴리오는 콘텐츠가 발행 시점에 결정되고 방문자는 항상 최신 스냅샷을 본다. 정적 사이트 생성기로 전환하면:
...
│                    로컬 (Mac mini)                     │
│                                                      │
│  oxibuilder serve       oxibuilder blog new ...            │
│  (admin-web)         oxibuilder query "SELECT ..."      │
│       │                     │                        │
│       └───────┬─────────────┘                        │
│               ▼                                       │
│          SQLite DB                                   │
...
| 관리 API | oxibuilder-core 상시 서버 | `oxibuilder serve` 로컬 서버 (불변) |
| 관리 UI | admin-web SPA → API | admin-web SPA → 로컬 API (불변) |
| CLI | HTTP 클라이언트 | HTTP 클라이언트 + 직접 SQLite 접근 |
| 공개 사이트 | oxibuilder-core가 SSR + SPA 서빙 | 정적 HTML + JSON + JS |
...
`oxibuilder build`가 각 확장의 `build_search_docs()`를 호출해 문서 벡터를 수집하고,
...
[
  {
    ...
    "body_preview": "Oxibuilder v2는 정적 사이트 생성기로...",
    "type": "blog",
    "url": "/blog/my-post",
...
| **`oxibuilder build`** | `/data/media/` → `out/media/` 복사만 |
| **`oxibuilder build --optimize`** | 추가로 webp 변환, srcset 생성 |
...
```
oxibuilder deploy [--target <target>] [--site <name>]
```
...
```
1. gh auth status 확인
2. git worktree add /tmp/oxibuilder-deploy gh-pages
3. cp -r out/* → worktree
4. git commit -m "deploy: $(date -Iseconds)"
5. git push origin gh-pages
6. git worktree remove /tmp/oxibuilder-deploy
```

## 8. 프리뷰

```
oxibuilder serve --preview    # out/ 디렉토리를 로컬 HTTP 서빙
```

`file://` 프로토콜은 CORS와 ES 모듈 로딩을 깨므로 반드시 HTTP 서버가 필요하다.
`oxibuilder serve`에 `--mode preview`를 추가해 `admin-web` 대신 `out/`을 정적 서빙.

## 9. 외부 API / 캐시 갱신

`oxibuilder build`는 외부 API를 호출하지 않는다. 빌드는 순수 DB → 정적 파일 변환만.
외부 데이터 수집은 별도 명령으로 분리:

```
oxibuilder cache refresh [--extension <name>]
...
```
oxibuilder build              # 전체 빌드
oxibuilder build --incremental  # 변경된 콘텐츠만 (v2.1에서)
```
...
```
oxibuilder build --site blog        → out/blog/
oxibuilder build --site portfolio   → out/portfolio/
oxibuilder deploy --site blog       → blog.oxinot.dev
```

`oxibuilder build`(site 생략)는 기본 사이트를 빌드.
...
```
oxibuilder query "<SQL>" [--json]
```

- CLI 바이너리가 `oxibuilder.db`를 직접 열어(`sqlx`) 쿼리 실행
- **읽기 전용** — INSERT/UPDATE/DROP은 차단
- 결과는 `--json` 플래그로 JSON 배열 출력

```bash
$ oxibuilder query "SELECT slug, title FROM blog_posts WHERE tags LIKE '%rust%'" --json
...
```
oxibuilder schema [--extension <name>] [--json]
```

```bash
$ oxibuilder schema --extension blog --json
{
  "tables": [
    {
      "name": "blog_posts",
...
AI 에이전트가 DB 구조를 모르더라도 `oxibuilder schema --json` 한 번으로 모든 걸 파악 가능.

### 13.3 Diff (v2.1)

```
oxibuilder blog diff <slug> [--json]
```

낮은 우선순위. DB에 리비전 이력이 없으면 현재 버전만 표시.
에이전트가 `oxibuilder blog show`로 현재 내용을 읽고 직접 이전/이후를 비교할 수 있으므로 v2 초기에는 없어도 무방.

### 13.4 신규 서브커맨드

```
oxibuilder build [--site <name>] [--optimize] [--incremental]
oxibuilder deploy [--target github|cloudflare|netlify] [--site <name>]
oxibuilder query "<SQL>" [--json]
oxibuilder schema [--extension <name>] [--json]
oxibuilder cache refresh [--extension <name>]
...
| # | 기능 | 우선순위 | 상태 |
|---|---|---|---|
| 1 | `oxibuilder query --json` | **필수** | 신규 |
| 2 | `oxibuilder schema --json` | **필수** | 신규 |
| 3 | `oxibuilder blog diff --json` | 있으면 좋음 | v2.1 |
| 4 | 배치 `blog new --file posts/*.md` | 있으면 좋음 | v2.1 |
| 5 | `oxibuilder status --json`에 최근 작업 내역 | 있으면 좋음 | v2.1 |

### 14.2 에이전트 워크플로우

```bash
# 1. DB 구조 파악
oxibuilder schema --json

# 2. 자유 질의
oxibuilder query "SELECT slug, title FROM blog_posts
  WHERE published_at IS NOT NULL ORDER BY published_at DESC" --json

# 3. 콘텐츠 읽기
oxibuilder blog show my-post --json

# 4. 편집
cat > /tmp/body.md << 'EOF'
수정된 본문...
EOF
oxibuilder blog edit my-post --file /tmp/body.md --json

# 5. 빌드 & 배포
oxibuilder build && oxibuilder deploy
```
...
| 컴포넌트 | 변경 |
|---|---|
| `oxibuilder-core` | `BuildExt` 트레이트 추가, 빌드 파이프라인 (rayon 병렬), `cache refresh` 분리 |
| 각 `oxibuilder-ext-*` | `BuildExt` 구현체 추가 (9개 확장) |
| `oxibuilder-cli` | `query`, `schema`, `build`, `deploy`, `cache`, `serve --preview` 서브커맨드 추가 |
| `web/` | 데이터 페칭을 정적 JSON으로 전환 (TanStack Query `queryFn` 변경 + `VITE_DATA_MODE`) |
| `admin-web/` | 불변 |
| `oxibuilder-server` | `serve --preview` 모드 추가 |
| SQLite 스키마 | 불변 |
| 배포 모델 | 상시 서버 → 정적 파일 (`oxibuilder deploy` → GitHub Pages 1순위) |
