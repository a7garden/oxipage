# Oxibuilder Extension SDK — building a new extension (doc/01 §1.4)

An Oxibuilder extension is a Cargo workspace member crate that implements the
`oxibuilder_core::extension::Extension` trait. This guide walks through building one from scratch.

## 1. Crate scaffold

```
crates/oxibuilder-ext-myfeature/
├── Cargo.toml
...
```toml
[package]
name = "oxibuilder-ext-myfeature"
version = "0.1.0"
edition = "2024"

[lib]
name = "oxibuilder_ext_myfeature"
path = "src/lib.rs"
...
2. **FTS5 shared index:** on publish, upsert into the shared index via
   `oxibuilder_core::search::upsert(pool, "myfeature", &doc_id, &title, &body, lang, published_at)`.
...
5. **Errors:** use `oxibuilder_core::error::ApiError` (`new` / `validation` / `internal`). The response
...
9. **External API keys:** the `[integrations]` section of `oxibuilder.toml` holds the *env-var name*;
...
이전(v1)에는 코어(`oxibuilder-core/src/setup.rs`)가 직접 `INSERT INTO blog_post` SQL을 작성했다. 이제는 blog 확장이 자기 도메인 데이터를 자기 시점에 시드한다 — **코어가 blog 테이블의 존재를 모른다**.
...
Add the dependency to `crates/oxibuilder-console/Cargo.toml` and add one line —
...
auth — write-route tests just exercise the handler. `oxibuilder-ext-blog` and
`oxibuilder-ext-projects` are the references.
