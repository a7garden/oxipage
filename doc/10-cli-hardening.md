...
| `oxibuilder deploy` | N/A | ❌ | Phase 5 의존 |
...
**이 계획이 다루는 것:** resolve chain 테스트, Client timeout/수명주기, commands.rs 분할, OXIBUILDER_SITE env 에러 처리, --insecure, E2E smoke test.

**별도 계획이 필요한 것:** 위 표의 모든 누락 명령군 — 각 extension의 API 스펙에 맞춘 CLI 명령 설계.

## 10.1 동기

`oxibuilder-cli` crate 1623 LOC 중 `sites.rs`(274 LOC)에만 10건의 단위 테스트가 있고, 나머지 4개 모듈(`commands.rs` 1009 LOC, `client.rs`, `credentials.rs`, `output.rs`)은 테스트 커버리지 0%다. 특히 `resolve_endpoint`(7단계 우선순위 체인)와 `Client::request`(HTTP 전송)는 한 단계의 우선순위 오류나 timeout 누락만으로도 명령이 엉뚱한 서버로 발행되거나 CLI가 무한 블록되는 결함으로 이어진다.
...
| `OXIBUILDER_SITE` env가 unknown 사이트를 조용히 무시 → default 폴백 | `commands.rs:264-269` | **낮음** — 침묵 동작, 추적 어려움 |
...
pub fn new(endpoint: String, token: Option<String>, insecure: bool) -> anyhow::Result<Self> {
    ...
    let mut builder = reqwest::Client::builder()
        .user_agent(concat!("oxibuilder-cli/", env!("CARGO_PKG_VERSION")))
...
```rust
#[arg(long, global = true, env = "OXIBUILDER_TLS_INSECURE")]
pub insecure: bool,
```

`dispatch()`에서 `Client::new(endpoint, token, cli.insecure)?` 로 전달. TLS 검증 스킵은 `--insecure` 또는 `OXIBUILDER_TLS_INSECURE=1` env로만 활성화되며, 기본값은 검증 유지.

### 10.3.4 `OXIBUILDER_SITE` env unknown 처리

`resolve_site_name()` 에서 OXIBUILDER_SITE env에 지정된 사이트가 존재하지 않으면, default_site로 조용히 폴백하지 않고 **에러**를 반환한다. flag와 동일한 수준의 명시적 의도로 취급한다.

```rust
// commands.rs — resolve_site_name() OXIBUILDER_SITE 분기 변경
// Before:
...
if let Ok(env) = std::env::var("OXIBUILDER_SITE") {
    if !env.is_empty() {
        ...
        anyhow::bail!("site '{env}' (from OXIBUILDER_SITE env) not found — use `oxibuilder site add` to create it");
    }
}
```
...
```
--endpoint <url>        OXIBUILDER_ENDPOINT       API server base URL
--token <tok>           OXIBUILDER_TOKEN          Bearer token
--site <name>           OXIBUILDER_SITE           Active site profile
--insecure              OXIBUILDER_TLS_INSECURE   Accept invalid TLS certs
--json                                        JSON output mode
--config <path>         OXIBUILDER_CONFIG         oxibuilder.toml path
```
...
**Files:**
- Modify: `crates/oxibuilder-cli/src/client.rs:28-39`
- Modify: `crates/oxibuilder-cli/src/main.rs` — `--insecure` flag 추가
- Modify: `crates/oxibuilder-cli/src/commands.rs:174-247` — dispatch() 시그니처
...
- [ ] **Step 5: cargo build -p oxibuilder-cli → clean compile 확인**

- [ ] **Step 6: Commit**

```bash
git add crates/oxibuilder-cli/src/client.rs crates/oxibuilder-cli/src/main.rs crates/oxibuilder-cli/src/commands.rs
...
**Files:**
- Create: `crates/oxibuilder-cli/src/commands/test_resolve.rs` (또는 commands.rs 하단 `#[cfg(test)] mod tests`)
- Modify: `crates/oxibuilder-cli/src/commands.rs:264-269` — OXIBUILDER_SITE env unknown → error

**Interfaces:**
- Consumes: `resolve_endpoint`, `resolve_token`, `resolve_site_name` (Task 1 이후 시그니처)
- Produces: 테스트 커버리지 (최소 15 test case)

- [ ] **Step 1: OXIBUILDER_SITE unknown → error 수정**

```rust
// commands.rs:264-272 — resolve_site_name() OXIBUILDER_SITE 분기
if let Ok(env) = std::env::var("OXIBUILDER_SITE") {
    if !env.is_empty() {
        if sites_file.exists(&env) {
...
mod tests {
    ...
    #[test]
    fn test_endpoint_oxibuilder_env() {
        let sites = empty_sites();
        // OXIBUILDER_ENDPOINT env → fallback
        std::env::set_var("OXIBUILDER_ENDPOINT", "https://env.example.com");
        let result = resolve_endpoint(None, None, &sites, None).unwrap();
        assert_eq!(result, "https://env.example.com");
        std::env::remove_var("OXIBUILDER_ENDPOINT");
    }
...
- [ ] **Step 5: `cargo test -p oxibuilder-cli` → 23+ 통과 확인 (기존 10 + 신규 15+)**

- [ ] **Step 6: Commit**

```bash
git add crates/oxibuilder-cli/src/commands.rs
git commit -m "fix(cli): OXIBUILDER_SITE unknown → error; add resolve chain unit tests (15+ cases)"
```
...
**Files:**
- Create: `crates/oxibuilder-cli/src/commands/mod.rs`
- Create: `crates/oxibuilder-cli/src/commands/site.rs`
- Create: `crates/oxibuilder-cli/src/commands/blog.rs`
- Create: `crates/oxibuilder-cli/src/commands/project.rs`
- Create: `crates/oxibuilder-cli/src/commands/link.rs`
- Create: `crates/oxibuilder-cli/src/commands/lobby.rs`
- Create: `crates/oxibuilder-cli/src/commands/extension.rs`
- Create: `crates/oxibuilder-cli/src/commands/backup.rs`
- Create: `crates/oxibuilder-cli/src/commands/auth.rs`
- Create: `crates/oxibuilder-cli/src/commands/init_status_serve.rs`
- Modify: `crates/oxibuilder-cli/src/main.rs` — 모듈 경로 변경
- Delete: `crates/oxibuilder-cli/src/commands.rs` (분할 완료 후)
...
```bash
mkdir -p crates/oxibuilder-cli/src/commands
git mv crates/oxibuilder-cli/src/commands.rs crates/oxibuilder-cli/src/commands/mod.rs
```
...
- [ ] **Step 12: `cargo build -p oxibuilder-cli` → clean compile 확인**

- [ ] **Step 13: `cargo test -p oxibuilder-cli` → 기존 23개 모두 통과 확인**

- [ ] **Step 14: Commit**

```bash
git add -A crates/oxibuilder-cli/src/commands/
git add crates/oxibuilder-cli/src/main.rs
...
**Files:**
- Create: `crates/oxibuilder-cli/tests/client_tests.rs`

**Interfaces:**
- Consumes: `Client` (Task 1 시그니처)
- Produces: HTTP-level integration tests

테스트는 실제 HTTP 서버를 띄우지 않고 `Client::new`의 validation + `ApiError` 구조체 round-trip을 검증한다. (full HTTP mock은 scope 밖 — oxibuilder-core의 test server 기반 통합 테스트로 전환 고려)

- [ ] **Step 1: Client 생성 basic test**

```rust
// crates/oxibuilder-cli/tests/client_tests.rs
use oxibuilder_cli::client::Client;
...
fn test_api_error_display() {
    let err = oxibuilder_cli::client::ApiError {
        status: 404,
        code: "not_found".into(),
...
- [ ] **Step 3: `cargo test -p oxibuilder-cli` → 27+ 통과 확인 (23 + 4 신규)**

- [ ] **Step 4: Commit**

```bash
git add crates/oxibuilder-cli/tests/
...
**Files:**
- Modify: `crates/oxibuilder-cli/src/main.rs` — `pub use` 노출
- Modify: `crates/oxibuilder-cli/src/client.rs` — `ApiError` public 확인

**Interfaces:**
- Produces: `oxibuilder_cli::client::Client`, `oxibuilder_cli::client::ApiError` 외부 접근 가능
...
**Files:**
- Create: `crates/oxibuilder-cli/tests/smoke.rs`

**Interfaces:**
- Consumes: 빌드된 `oxibuilder` 바이너리
- Produces: E2E 동작 확인

실행할 `oxibuilder` 바이너리를 찾기 위해 `cargo build -p oxibuilder-cli` 의 출력물 경로를 사용한다. 실제 HTTP 서버를 띄우지 않고 `--help`, `--version`, `site` 서브커맨드만 검증.

- [ ] **Step 1: CLI help/version smoke test 작성**

```rust
// crates/oxibuilder-cli/tests/smoke.rs
use std::process::Command;

fn oxibuilder() -> Command {
    Command::new(env!("CARGO_BIN_EXE_oxibuilder"))
}

#[test]
fn test_help() {
    let out = oxibuilder().arg("--help").output().unwrap();
...
fn test_site_list_empty() {
    let out = oxibuilder().arg("site").arg("list").output().unwrap();
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn test_site_add_list_rm_flow() {
    ...
    let config_dir = dirs::config_dir().unwrap().join("oxibuilder");
...
- [ ] **Step 2: `cargo test -p oxibuilder-cli` → E2E 포함 전체 통과 확인**

- [ ] **Step 3: Commit**

```bash
git add crates/oxibuilder-cli/tests/smoke.rs
git commit -m "test(cli): add E2E smoke tests (help, site add/list/rm flow)"
```

---

### Task 7: 최종 검증 + 사이트 테스트 추가 (OXIBUILDER_SITE env E2E)

**Files:**
- Modify: `crates/oxibuilder-cli/tests/smoke.rs` — env 테스트 추가

- [ ] **Step 1: OXIBUILDER_SITE env unknown → error E2E 검증**
...
- [ ] **Step 2: OXIBUILDER_SITE env unknown → error 검증**

```rust
#[test]
fn test_oxibuilder_site_env_unknown_errors() {
    let out = oxibuilder()
        .env("OXIBUILDER_SITE", "nonexistent-env")
        .arg("site").arg("list")
        .output().unwrap();
    assert!(!out.status.success());
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("OXIBUILDER_SITE"));
    assert!(stderr.contains("not found"));
}
```

- [ ] **Step 3: `cargo test -p oxibuilder-cli` → 전체 30+ 통과 확인**

- [ ] **Step 4: `cargo build -p oxibuilder-cli` → clean, no warnings 확인**

- [ ] **Step 5: Commit**

```bash
git add crates/oxibuilder-cli/tests/smoke.rs
git commit -m "test(cli): add OXIBUILDER_SITE env error E2E test"
```
...
| `--insecure` | 없음 | `OXIBUILDER_TLS_INSECURE` / `--insecure` |
| `OXIBUILDER_SITE` unknown | silent fallback | error exit 1 |
| 테스트 건수 | 10 | 30+ |
