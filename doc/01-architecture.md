...
> 
> - **Content management**: CLI + `oxipage serve` (admin-web) + SQLite — unchanged, runs locally
...
graph TB
    subgraph Local["로컬 (Mac mini)"]
        subgraph Management["관리 모드 (oxipage serve)"]
...
| 모드 | 명령 | 목적 |
|---|---|---|
| 관리 | `oxipage serve` | admin-web + API 서버 (로컬 전용) |
