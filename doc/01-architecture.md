...
> 
> Oxibuilder pivots from a dynamic server-side rendered application to a **Static Site Generator**.
> 
> - **Content management**: CLI + `oxibuilder console` (admin-web) + SQLite — unchanged, runs locally
> - **Public site**: generated by `oxibuilder build` → static HTML + JSON + JS → deployed via `oxibuilder deploy` → no runtime server
...
graph TB
    subgraph Local["로컬 (Mac mini)"]
        subgraph Management["관리 모드 (oxibuilder console)"]
            CLI["Oxibuilder CLI<br/>(로컬 AI 에이전트)"] --> |HTTP API| Core["oxibuilder-core (Axum)<br/>인증 · 미디어 업로드 · 백그라운드 잡"]
        ...
        subgraph Build["빌드/배포 모드 (oxibuilder build && deploy)"]
    ...
    subgraph CDN["GitHub Pages / Cloudflare Pages"]
        out -->|oxibuilder deploy| Static["순수 정적 파일 서빙<br/>(서버 불필요)"]
    end
```
...
graph TB
    ...
    subgraph Host["Mac mini M4 — macOS 26 (Tahoe)"]
        subgraph Process["oxibuilder-core 단일 프로세스"]
            Core["oxibuilder-core (Axum)<br/>인증 · 미디어 · 검색 · 스케줄러 · 정적 자산 서빙"]
            Ext1["ext: profile"]; Ext2["ext: projects"]; Ext3["ext: blog"]
            Ext4["ext: novels"]; Ext5["ext: scraps"]; Ext6["ext: activity"]
            Ext7["ext: movies"]; Ext8["ext: books"]; Ext9["ext: links"]
            Core --- Ext1 & Ext2 & Ext3 & Ext4 & Ext5 & Ext6 & Ext7 & Ext8 & Ext9
    ...
    CLI["Oxibuilder CLI"] -->|REST API, Bearer Token| Caddy
```

## 1.2 코어와 확장의 경계

**코어(`oxibuilder-core`)가 제공하는 것:**
...
- 설정 로딩 (`oxibuilder.toml` / 환경변수)
- SSR 스냅샷 렌더러 (SEO/OG 카드용, §1.6)
- **빌드 파이프라인 (v2):** `BuildExt` 레지스트리, rayon 병렬 처리, 정적 HTML + JSON 파일 출력
- **배포 명령 (v2):** `oxibuilder deploy` — GitHub Pages 1순위 (git worktree 기반)
- 관측성 (구조화 로깅 via `tracing`, `/healthz`, graceful shutdown, 부팅 시 `oxibuilder.toml`/환경변수 검증 및 **필수 외부 API 키 누락 경고**)
...
**확장(`oxibuilder-ext-*`)이 제공하는 것:**

- 자기 도메인의 DB 스키마 + 마이그레이션
- 자기 도메인의 REST 라우트 (`/api/console/blog/**` 등)
- 자기 도메인의 CLI 서브커맨드 (`oxibuilder blog ...`)
...
├── crates/
│   ├── oxibuilder-core/             # 서버 부트스트랩, 인증, 미디어, 검색, 스케줄러, 확장 레지스트리, **BuildExt, 빌드 파이프라인**
│   ├── oxibuilder-cli/               # CLI 바이너리 (관리 + 빌드 + 배포 + 쿼리)
│   ├── oxibuilder-ext-profile/
│   ├── oxibuilder-ext-projects/
│   ├── oxibuilder-ext-blog/
│   ├── oxibuilder-ext-novels/
│   ├── oxibuilder-ext-scraps/
│   ├── oxibuilder-ext-activity/
│   ├── oxibuilder-ext-movies/
│   ├── oxibuilder-ext-books/
│   └── oxibuilder-ext-links/
...
│   ├── deploy.yaml                  # Oxibuilder 자체 배포 매니페스트 (컨테이너/볼륨/환경변수 선언)
│   └── Caddyfile.example
├── .agent/skills/oxibuilder/SKILL.md   # oh-my-pi 등 에이전트용 스킬
└── docs/                            # 본 설계 문서
```
...
pub trait Extension: Send + Sync {
    /// 고유 식별자. oxibuilder.toml의 enabled 목록, API 경로 프리픽스, 로비 매니페스트 키로 재사용됨
...
> **v2 변경 (2026-07-28):** SSR 스냅샷을 빌드 시점으로 확장합니다. `oxibuilder build`가 모든 published 콘텐츠의 정적 HTML을 일괄 생성하고, React SPA가 그 위에서 하이드레이트됩니다. 개별 publish 시점 스냅샷 생성 대신 `BuildExt.build_pages()`가 전체 목록을 생성합니다.
...
정적 사이트 아키텍처로 전환하면서 인증 시스템을 제거했습니다. 관리 서버(`oxibuilder console`)는 기본 loopback(127.0.0.1)에서만 동작하며, 모든 API 라우트가 인증 없이 공개되어 있습니다. 원격 노출이 필요한 경우 사용자가 명시적으로 `host`를 변경해야 하며, 그에 따른 보안 책임은 사용자에게 있습니다.
...
모든 외부 API 연동은 **키가 없으면 조용히 비활성화**되어야 합니다(예: `OXIBUILDER_TMDB_KEY` 미설정 시 영화 확장은 수동 입력만 지원). 이는 OSS 사용자가 원하는 통합만 골라 쓸 수 있게 하기 위한 원칙입니다.

> **v2 변경 (2026-07-28):** 상시 서버를 가정한 백그라운드 잡은 `oxibuilder cache refresh` 명령으로 대체됩니다. 빌드 시점에 외부 API를 호출하지 않고, 별도 명령으로 수집된 데이터를 DB에 기록한 후 `build`가 읽습니다.
...
| 모드 | 명령 | 목적 |
|---|---|---|
| 관리 | `oxibuilder console` | admin-web + API 서버 (로컬 전용) |
| 빌드 | `oxibuilder build` | DB → 정적 HTML + JSON 생성 |
| 배포 | `oxibuilder deploy` | 정적 파일 → GitHub Pages/Cloudflare Pages/Netlify |
| 캐시 | `oxibuilder cache refresh` | 외부 API(GitHub, TMDB, 알라딘) 수집 |
| SQL 질의 | `oxibuilder query "<SQL>"` | AI 에이전트용 직접 DB 조회 |
...
`oxibuilder deploy --target github-pages`가 내부적으로 git worktree를 사용해 `gh-pages` 브랜치에 `out/`을 푸시합니다. 서버 프로세스 관리(launchd/systemd)가 더 이상 필요하지 않습니다.
