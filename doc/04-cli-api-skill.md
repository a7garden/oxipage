...
`oxibuilder` CLI가 하는 모든 일은 HTTP API 호출입니다. 인증은 폐지되었으므로 토큰이 필요 없습니다(단, `oxibuilder console`로 로컬 관리 서버를 띄우는 것 자체는 예외 — 이때는 CLI가 곧 서버 프로세스를 기동하는 것). 이렇게 하면:
...
```text
# 프로젝트 관리
oxibuilder init                                   # oxibuilder.toml 스캐폴딩
oxibuilder status [--json]                         # 초안/최근 게시물/배포 상태 요약
oxibuilder console [--port 8787]                     # 로컬 개발 서버
oxibuilder deploy                                   # 컨테이너 빌드 + Mac mini 배포 (5장)
oxibuilder backup export [--out FILE]

# 프로필(명함)
oxibuilder profile edit                             # $EDITOR로 profile.toml 열람/저장
oxibuilder profile show [--json]

# 프로젝트 포트폴리오
oxibuilder project add --title-ko "..." --title-en "..." \
    [--desc-ko FILE] [--desc-en FILE] [--tech rust,react] \
    [--screenshot PATH ...] [--link repo=URL --link demo=URL]
oxibuilder project publish <slug>
oxibuilder project list [--status active|archived|wip] [--json]

# 블로그
oxibuilder blog new "제목" [--lang ko|en] [--file DRAFT.md]
oxibuilder blog publish <slug>
oxibuilder blog list [--draft] [--json]

# 소설
oxibuilder novel new "제목"
oxibuilder novel chapter add <novel-slug> --title "1화" --file ch1.md
oxibuilder novel chapter publish <novel-slug> <order>

# 영화/시리즈 리뷰
oxibuilder review movie add --tmdb "<검색어 또는 tmdb id>" \
    --rating 8.5 [--review-file review.md] [--series "해리포터"]
oxibuilder review movie series create "해리포터" [--rating 9.0] [--review-file r.md]

# 도서 리뷰
oxibuilder review book add --isbn "<isbn 또는 검색어>" \
    --rating 4.5 [--review-file review.md] [--status reading|completed]

# 스크랩
oxibuilder scrap add <url> [--source hn|geeknews|manual] [--note "..."]
oxibuilder scrap queue                              # 백그라운드로 수집된 추천 후보 목록

# 생태계 링크
oxibuilder link add --title "..." --url "..." [--desc-ko "..."] [--desc-en "..."]

# 활동
oxibuilder activity sync                            # 즉시 GitHub 동기화 트리거

# 로비/확장
oxibuilder lobby layout set <extension> --mode canvas|grid|list
oxibuilder extension list
oxibuilder extension enable|disable <name>
oxibuilder extension purge <name> --yes               # 테이블 DROP + 미디어 rm (doc/02 §2.13)
oxibuilder extension install <name>                 # Phase 5, OSS 레지스트리 (6장)
```
...
## 4.4 설정 파일 `oxibuilder.toml`

```toml
[site]
name = "내 이름의 Oxibuilder"
...
tmdb_api_key_env = "OXIBUILDER_TMDB_KEY"
aladin_ttbkey_env = "OXIBUILDER_ALADIN_TTBKEY"
...
oh-my-pi(omp)는 `SKILL.md` 기반 스킬을 여러 위치에서 읽는데, 그중 omp 고유의 정식 위치는 저장소의 `.agent/skills/` (또는 `.agents/skills/`) 이며, `.claude/skills/`류 기존 관례 위치도 함께 인식합니다. 따라서 Oxibuilder 저장소 루트에 아래처럼 배치합니다.
...
name: oxibuilder-cli
description: >
  개인 홈페이지(Oxibuilder)에 콘텐츠를 발행할 때 사용하는 CLI 스킬입니다.
...
- 이 스킬을 쓰려면 oh-my-pi 실행 환경에 Oxibuilder PAT가 **사전 프로비저닝**되어 있어야 합니다:
  1. 오너가 로컬에서 `oxibuilder auth token create --label omp-agent --scopes post:write` (에이전트엔 `post:publish`를 주지 않습니다 — 초안 우선 원칙).
  2. 발급받은 평문 토큰을 oh-my-pi 환경변수 `OXIBUILDER_TOKEN`으로 주입. 평문은 이때 한 번만 보이므로 즉시 환경에 넣으세요.
  3. CLI는 `OXIBUILDER_TOKEN`이 있으면 자동으로 `Authorization: Bearer` 헤더에 붙입니다. 없으면 모든 쓰기 명령이 401로 실패합니다.
...
2. `oxibuilder blog new "<제목>" --lang ko --file <임시파일> --json`
3. 결과의 `data.slug`를 사용자에게 알려주고, 게시 여부를 물음
4. 게시 승인 시에만 `oxibuilder blog publish <slug> --json`

### 영화 리뷰
1. `oxibuilder review movie add --tmdb "<제목>" --json` 으로 후보 검색
2. 여러 후보가 반환되면 사용자에게 선택지를 보여주고 확인받음
3. 평점/리뷰 텍스트가 없으면 사용자에게 물어봄(임의로 지어내지 않음)
4. 시리즈 언급이 있으면(`--series "<시리즈명>"`) 기존 시리즈 그룹 존재 여부를
   `oxibuilder review movie series list --json`으로 먼저 확인

### 스크랩
1. URL만 주어지면 `oxibuilder scrap add <url> --json`으로 메타데이터만 우선 저장
