...
graph LR
    ...
    Caddy -->|HTTP, 컨테이너 전용 IP| App[oxipage 컨테이너<br/>Apple container 런타임]
...
- **기본 실행 모델:** `oxipage-console` 바이너리를 launchd(macOS, `deploy/oxipage.plist.example`) 또는 systemd(Linux, `deploy/oxipage.service.example`)로 직접 기동하는 것이 기본 경로입니다. container를 쓰는 경우에도 Caddy→컨테이너 흐름은 동일합니다. OSS 사용자(§5.7)가 Apple Silicon/macOS 없이도 Linux 서버 한 대에서 동일하게 운영할 수 있는 것도 이 경로 덕분입니다.
...
WORKDIR /app
COPY . .
RUN cargo build --release -p oxipage-console

FROM debian:stable-slim
COPY --from=build /app/target/release/oxipage-console /usr/local/bin/oxipage-console
# 프론트엔드(web/dist)는 rust-embed로 이미 바이너리에 내장되어 별도 COPY 불필요
EXPOSE 8787
ENTRYPOINT ["/usr/local/bin/oxipage-console"]
```

```bash
container build -t oxipage:latest -f deploy/Dockerfile .

container run -d \
  --name oxipage \
  -v ~/oxipage-data/db:/data/db \
  -v ~/oxipage-data/media:/data/media \
  --env-file ~/oxipage-data/.env \
  oxipage:latest
```

- `rust-embed` 크레이트로 `web/dist`(Vite 빌드 산출물)를 Rust 바이너리에 컴파일 타임 내장 → **이미지 하나 = 프론트엔드 + 백엔드 전부.**
- `.env`에는 `OXIPAGE_TMDB_KEY`, `OXIPAGE_ALADIN_TTBKEY`, `OXIPAGE_ADMIN_PASSWORD_HASH` 등 비밀 값만 둡니다.
- `oxipage deploy` CLI 명령(4장)은 이 빌드+실행 시퀀스를 감싸는 얇은 래퍼입니다.
...
- **수동 스냅샷:** `oxipage backup export`로 DB + media를 tar로 묶어 임의 시점 백업(예: 큰 구조 변경 전)
...
2. **`deploy/deploy.yaml`(Oxipage 자체 매니페스트)은 지금부터 단일 프로세스 배포의 단일 진실 소스로 씁니다** — 서비스·볼륨·환경을 선언하고, `oxipage deploy`는 이를 읽어 (a) 바이너리 직접 기동(launchd/systemd) 또는 (b) `container run` 중 선택한 모드로 실행합니다. compose-spec과 유사한 키 이름을 써서, Apple이 공식 compose를 추가하면 표준 `compose.yaml`로 거의 그대로 변환 가능합니다.

## 5.7 OSS 셀프호스팅 제품으로의 일반화 경로

**전제 재확인(§0.3):** 멀티테넌트 SaaS가 아니라 "각자 자기 인스턴스를 돌린다" 모델을 유지합니다. 이 결정 덕분에 일반화 작업은 생각보다 작습니다 — 이미 설정 기반(`oxipage.toml`)이고 확장은 켜고 끌 수 있으니, 남은 일은 아래 정도입니다.

1. **개인화 요소 제거:** 이름/문구 등 하드코딩된 부분을 전부 `oxipage.toml` 또는 `profile`로 이동
2. **확장 레지스트리:** 초기에는 GitHub 저장소 하나에 curated JSON 인덱스(`{name, repo, description, version}[]`)를 두는 정도로 단순하게 시작 — Homebrew tap과 비슷한 방식. `oxipage extension install <name>`이 이 인덱스를 조회해 설치
3. **템플릿 저장소:** `oxipage-starter` 레포를 만들어 `git clone` + 원클릭 설치 스크립트(`curl ... | sh`)로 남이 바로 시작할 수 있게 함
