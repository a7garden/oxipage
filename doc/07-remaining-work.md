...
| Oxibuilder CLI(init/status/serve/auth/blog/project/link/lobby/backup/site/admin) + Dynamic 확장 명령 | ✅ |
| 배포 템플릿·LICENSE(MIT)·SDK 문서·레지스트리·starter | ✅ |
| 멀티사이트 (sites.toml CRUD, --site flag, OXIBUILDER_SITE env, 3단계 fallthrough) | ✅ |
| CLI 확장성 (doc/11: external_subcommand + 5개 확장 CLI + server discovery) | ✅ |
| 관리 콘솔 (oxibuilder-admin + admin-web SPA + proxy/themes/sites API) | ✅ |
...
| v0 임시 인증 → PAT 체계 | `OXIBUILDER_ADMIN_TOKEN` 단일 토큰 + constant-time 비교. `doc/01` §1.8의 PAT(scope: `post:write`/`post:publish`, SHA-256 해시 저장, `oxibuilder auth token create/list/revoke`)로 교체. | Phase 1/4 |
| CLI 크레이트 | Phase 0 범위에서 의도적 제외. `oxibuilder` CLI 바이너리(`doc/04` §4.3 명령 체계) 신규 추가. | Phase 1 |
...
**작업 항목:**
- `oxibuilder-ext-blog`: `blog` 테이블(`doc/02` §2.6), `lang`/`translation_group_id` 느슨한 번역 연결, 초안(`published_at` NULL)/발행 상태, 태그(JSON 배열). 라우트 `GET/POST /api/console/blog/posts`, `GET/PATCH/DELETE /api/console/blog/posts/{slug}`.
- `oxibuilder-ext-projects`: `doc/02` §2.4 — `title_ko`/`title_en`, `description_ko`/`description_en`(구조적 이중언어 강제), `tech_stack`, `status`, `links`, `featured`, `screenshots` 테이블.
- `oxibuilder-ext-links`: `doc/02` §2.11 — `LinkCard` + `display_order` + `featured`.
...
- `oxibuilder-cli` 크레이트 신규: `doc/04` §4.3 — `auth`(login/token create/list/revoke), `init`, `status`, `serve`, `blog`, `project`, `link`. **원칙: CLI가 하는 모든 일은 인증된 HTTP 호출**(`doc/04` §4.1). PAT 읽기: `OXIBUILDER_TOKEN` 환경변수 → 키체인/`~/.config/oxibuilder/credentials`(0600). `--json` 전역 지원.
...
**배포(완료 기준의 일부):**
- `deploy/Caddyfile.example`, `deploy/oxibuilder.plist.example`(macOS launchd)/`deploy/oxibuilder.service.example`(Linux).
...
**작업 항목:**
- `oxibuilder-ext-movies` + TMDB 연동(`doc/02` §2.9): `MovieEntry` + `SeriesGroup`. TMDB 검색/캐시 흐름(sequence diagram). `OXIBUILDER_TMDB_KEY` 미설정 시 수동 입력만 지원(조용히 비활성화 원칙). TMDB attribution 푸터 표기.
- `oxibuilder-ext-books` + 알라딘 OpenAPI(1순위) / Google Books(폴백) 연동(`doc/02` §2.10). `OXIBUILDER_ALADIN_TTBKEY` 미설정 시 `manual`만.
- `oxibuilder-ext-novels`: `Novel` + `NovelChapter`(`char_count` 자동 계산, `doc/02` §2.5).
- `oxibuilder-ext-scraps` + HN(Firebase API)/GeekNews(RSS) 수집 잡(`doc/02` §2.7). **발행은 항상 사람 선택** — 백그라운드 잡은 "추천 큐"만 채운다.
- `oxibuilder-ext-activity` + GitHub 활동(`doc/02` §2.8): webhook 1순위 + Events API 15분 폴링(보조). **공개 Events API만 사용 → private repo 노출 안 됨(설계 보장)**.
...
- `oxibuilder lobby layout set <extension> --mode canvas|grid|list` CLI.
...
**작업 항목:**
- `.agent/skills/oxibuilder-cli/SKILL.md` 작성(`doc/04` §4.6 초안 그대로 시작점). oh-my-pi(GLM/MiniMax/DeepSeek 등 어떤 모델이든) 실사용 테스트.
...
- 개인화 요소 전부 `oxibuilder.toml`/`profile`로 이관.
- `oxibuilder-starter` 템플릿 저장소 + 원클릭 설치 스크립트(`curl … | sh`).
- 확장 레지스트리: GitHub 저장소 1개의 curated JSON 인덱스(Homebrew tap류). `oxibuilder extension install <name>`.
- `Extension` 트레이트 공개 SDK 문서화("새 확장 만들기" 가이드 1종).
- `oxibuilder deploy` 매니페스트(`deploy/deploy.yaml`, compose-spec 유사 키)를 단일 진실 소스로 — (a) 바이너리 직접 기동(launchd/systemd) 또는 (b) `container run` 선택.
...
- **확장 API 경로**: axum 0.8 `nest` 시맨틱상 루트 라우트는 prefix 무슬래시로 서빙됨(`/api/console/<ext>`, trailing slash 없음). `oxibuilder-cli` 및 프론트 fetch 경로도 이 규칙을 따를 것.
...
**목표:** 상시 서버 의존형(v1)에서 정적 사이트 생성기(v2)로 전환. `oxibuilder build && oxibuilder deploy`로 GitHub Pages에 배포 가능.
...
| 4 | `oxibuilder build` CLI 명령 | CLI | 3 |
| 5 | `oxibuilder deploy` CLI 명령 (GitHub Pages 1순위, git worktree 기반) | CLI | 4 |
| 6 | `oxibuilder query` / `oxibuilder schema` CLI 명령 (AI 에이전트용) | CLI | 없음 |
| 7 | `oxibuilder cache refresh` CLI 명령 (외부 API 수집, 빌드와 분리) | CLI | 없음 |
| 8 | `oxibuilder console --preview` (out/ 디렉토리 HTTP 서빙) | Server | 4 |
