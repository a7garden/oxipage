    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="border border-line rounded-lg overflow-hidden">
          ...
          {recent.map((row, i) => (
            <div key={str(field(row, "id") || field(row, "slug") || i)} className="flex border-t border-line text-sm hover:bg-surface/30 cursor-pointer">
              <div className="flex-1 px-4 py-2.5 truncate">{str(field(row, "title"))}</div>
              ...
              <div className="w-28 px-4 py-2.5 text-muted text-xs">
                {str(field(row, "published_at") || field(row, "updated_at"))}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex gap-2">
        ...
        <Button variant="outline" onClick={() => navigate(`/s/${slug}/deploy`)}><Rocket size={14} className="mr-1" /> Build & Deploy</Button>
      </div>
    </div>
  );
}
