...
function pathToStaticFile(path: string): string {
  // Extract first path segment to determine which JSON file to load
  // e.g. /blog/my-post → /data/blog.json, /search?q=rust → /data/search-index.json
  const p = path.split('?')[0];
  const segment = p.split('/').filter(Boolean)[0];
  if (!segment) return '/data/manifest.json';
  if (segment === 'search') return '/data/search-index.json';
  return `/data/${segment}.json`;
}
